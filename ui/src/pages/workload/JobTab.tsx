...
// 数据状态
  ...
  const loadWorkloads = useCallback(async () => {
    ...
    try {
      ...
      message.error(t('messages.fetchError', { type: 'Job' }));
    } finally {
      setLoading(false);
    }
    ...
    },
    {
      ...
      render: (record: WorkloadInfo) => (
        <Space size="small">
          ...
          <Popconfirm
            title={t('actions.confirmDelete', { type: 'Job' })}
  ...
  const handleBatchRedeploy = async () => {
    if (selectedRowKeys.length === 0) {
      message.warning(t('actions.selectRedeploy', { type: 'Job' }));
      return;
    }
    ...
    <div>
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => t('messages.totalItems', { count: total, type: 'Job' }),
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            <strong>{t('scale.workloadName', { type: 'Job' })}：</strong>{scaleWorkload?.name}
          </div>
          <div>
