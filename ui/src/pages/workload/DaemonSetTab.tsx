...
// 数据状态
  ...
  const loadWorkloads = useCallback(async () => {
    ...
    try {
      ...
      message.error(t('messages.fetchError', { type: 'DaemonSet' }));
    } finally {
      setLoading(false);
    }
    ...
    },
    {
      ...
      render: (record: WorkloadInfo) => (
        <Space size="small">
          ...
          <Popconfirm
            title={t('actions.confirmDelete', { type: 'DaemonSet' })}
  ...
  const handleBatchRedeploy = async () => {
    if (selectedRowKeys.length === 0) {
      message.warning(t('actions.selectRedeploy', { type: 'DaemonSet' }));
      return;
    }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        >
          {t('actions.create', { type: 'DaemonSet' })}
        </Button>
      </div>
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => t('messages.totalItems', { count: total, type: 'DaemonSet' }),
      ...
      {/* 扩缩容Modal */}
      <Modal
        title={t('scale.title', { type: 'DaemonSet' })}
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            <strong>{t('scale.workloadName', { type: 'DaemonSet' })}：</strong>{scaleWorkload?.name}
          </div>
          <div>
